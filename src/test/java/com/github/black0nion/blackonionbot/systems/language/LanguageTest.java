...
public class LanguageTest {
	...
	public void test_language_json_schema() {
		JSONObject jsonSchema = new JSONObject(new JSONTokener(
				Objects.requireNonNull(LanguageTest.class.getResourceAsStream("/translationsschema.json"))));
	...
	public void test_languages_against_json_schema() {
		...
		translations = new Reflections("translations", Scanners.Resources).getResources("[A-Z][a-z]+\\.json").stream()
				.map(resource -> new JSONObject(new JSONTokener(Objects
						.requireNonNull(Thread.currentThread().getContextClassLoader().getResourceAsStream(resource)))))
