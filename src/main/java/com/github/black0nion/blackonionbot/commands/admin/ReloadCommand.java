...
public class ReloadCommand extends SlashCommand {
  ...
  public ReloadCommand() {
    super(builder(Commands.slash("reload", "Reloads the bot").addSubcommands(
        ...
        new SubcommandData("method", "Reloads a specific method")
            .addOptions(new OptionData(OptionType.STRING, "method", "The method to reload", true)
                .addChoices(Utils.add(
                    ...
                    reloadableMethods.keySet().stream().map(m -> new Command.Choice(m, m))
                        .collect(Collectors.toList()),
                    new Command.Choice("ALL", "all")))))).setAdminGuild());
  }
  ...
  public static void initReloadableMethods() {
    ...
    new Reflections(Main.class.getPackage().getName(), Scanners.MethodsAnnotated)
        .getMethodsAnnotatedWith(Reloadable.class).forEach(method -> reloadableMethods
            .put(method.getAnnotation(Reloadable.class).value(), method));
  }
  ...
  public void execute(SlashCommandEvent cmde, SlashCommandInteractionEvent e, BlackMember member,
    ...
    if (e.getSubcommandName() == null)
      throw new NullPointerException("Subcommand name is null!");
    ...
    if (e.getSubcommandName().equals("list")) {
      ...
      for (final Map.Entry<String, Method> entry : reloadableMethods.entrySet()) {
        ...
        reloadableMethodsString.append("\n").append(entry.getKey()).append(": ")
            .append(meth.getDeclaringClass().getSimpleName()).append(".").append(meth.getName());
      }
