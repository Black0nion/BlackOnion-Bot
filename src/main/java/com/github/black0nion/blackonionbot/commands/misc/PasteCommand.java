...
public class PasteCommand extends TextCommand {
	...
	public PasteCommand() {
		this.setCommand("paste", "uploadtext").setSyntax("<text (codeblock with language specification if wanted)>")
		...
		cmde.loading(msg -> {
			try {
				HttpRequest.Builder requestBuilder = HttpRequest
						.newBuilder(URI.create("https://paste.sv-studios.net/documents"))
						.POST(HttpRequest.BodyPublishers.ofString(finalBody)).header("Content-Type", "text/plain");

				if (finalLanguage != null) {
					requestBuilder.setHeader("language", finalLanguage);
				}

				final HttpResponse<String> response = Bot.getInstance().getHttpClient().send(requestBuilder.build(),
				...
				final EmbedBuilder builder = cmde.success()
						...
						.setDescription("```" + (finalLanguage != null ? finalLanguage : "")).appendDescription("\n")
