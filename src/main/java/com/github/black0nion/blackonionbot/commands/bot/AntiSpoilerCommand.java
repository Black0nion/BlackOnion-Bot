...
public class AntiSpoilerCommand extends TextCommand {
	...
	public AntiSpoilerCommand() {
		this.setCommand("antispoiler", "as").setSyntax("[replace | delete | off]")
	...
	public void execute(final String[] args, final CommandEvent cmde, final MessageReceivedEvent e,
		...
		if (Utils.handleRights(guild, author, null, Permission.MESSAGE_MANAGE))
			return;
		if (args.length >= 2) {
			...
			if (type.equalsIgnoreCase("replace")) {
				...
				message.replyEmbeds(EmbedUtils.getSuccessEmbed(author, guild)
						.addField("antispoilerstatuschanged",
								cmde.getTranslation("antispoileris",
										new Placeholder("status", cmde.getTranslation("remove"))),
								false)
						.build()).queue();
				...
				message.replyEmbeds(EmbedUtils.getSuccessEmbed(author, guild)
						.addField("antispoilerstatuschanged",
								cmde.getTranslation("antispoileris",
										new Placeholder("status", cmde.getTranslation("delete"))),
								false)
						.build()).queue();
				...
				message.replyEmbeds(
						EmbedUtils.getSuccessEmbed(author, guild)
								.addField("antispoilerstatuschanged",
										cmde.getTranslation("antispoileris",
												new Placeholder("status", cmde.getTranslation("off"))),
										false)
								.build())
						.queue();
			} else {
			...
			message.replyEmbeds(
					EmbedUtils.getSuccessEmbed(author, guild)
							.addField(
									LanguageSystem.getTranslation("antispoilerstatus", author, guild).replace(
											"%status%",
											LanguageSystem.getTranslation(guild.getAntiSpoilerType().name(), author,
													guild)),
									LanguageSystem.getTranslation("howtoantispoilertoggle", author, guild).replace(
											"%command%", "``" + CommandEvent.getCommandHelp(guild, this) + "``"),
									false)
							.build())
					.queue();
		}
