...
public class AvatarCommand extends TextCommand {
  ...
  public void execute(final String[] args, final CommandEvent cmde, final MessageReceivedEvent e,
    ...
    if (args.length > 1) {
      ...
      if (!mentionedBlackUsers.isEmpty()) {
        ...
        e.getJDA().retrieveUserById(user.trim())
            .queue(uzer -> print(cmde, author, BlackUser.from(uzer)), failure -> cmde.exception());
        return;
      }
    }
  ...
  private static void print(final CommandEvent cmde, final BlackUser author,
      final BlackUser mentionedUser) {
