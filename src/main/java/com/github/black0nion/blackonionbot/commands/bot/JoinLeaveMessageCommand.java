...
public class JoinLeaveMessageCommand extends TextCommand {

  public JoinLeaveMessageCommand() {
    this.setCommand("joinleavemessage", "jlm").setRequiredPermissions(Permission.ADMINISTRATOR)
        .setRequiredArgumentCount(2).setSyntax("<join | leave> <message>");
  }
  ...
  public void execute(final String[] args, final CommandEvent cmde, final MessageReceivedEvent e,
    ...
    if (args[1].equalsIgnoreCase("join")) {
      ...
      cmde.success("setjoinmessage", "joinmessagesestto",
          new Placeholder("msg", "`" + newMessage + "`"));
    } else if (args[1].equalsIgnoreCase("leave")) {
      guild.setLeaveMessage(newMessage);
      cmde.success("setleavemessage", "leavemessagesestto",
          new Placeholder("msg", "`" + newMessage + "`"));
    } else
      cmde.sendPleaseUse();
  }
