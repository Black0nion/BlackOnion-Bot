...
public class ToggleCommand extends SlashCommand {
	...
	public void updateAutoComplete() {
		...
		this.updateAutoComplete("command", SlashCommandBase.commands.entrySet().stream()
				.filter(e -> ((currentCommand.setAndGet(e.getValue().getValue())).getRequiredCustomPermissions() == null
	...
	public void execute(SlashCommandEvent cmde, SlashCommandInteractionEvent e, BlackMember member, BlackUser author,
		...
		final SlashCommand command = SlashCommandBase.commands.get(e.getOption("command", OptionMapping::getAsString))
				.getValue();
		...
		if (newStatus == null) {
			cmde.send("commandstatus", new Placeholder("cmd", command.getName()),
					new Placeholder("status", cmde.getTranslation(guild.isCommandActivated(command) ? "on" : "off")));
		} else {
