...
public class ToggleCommand extends SlashCommand {
  ...
  public ToggleCommand() {
    super(builder(Commands.slash("toggle", "Toggle commands").addOptions(
  ...
  public void updateAutoComplete() {
    ...
    this.updateAutoComplete("command",
        SlashCommandBase.commands.entrySet().stream()
            .filter(e -> ((currentCommand.setAndGet(e.getValue().getValue()))
                .getRequiredCustomPermissions() == null
  ...
  public void execute(SlashCommandEvent cmde, SlashCommandInteractionEvent e, BlackMember member,
    ...
    final SlashCommand command = SlashCommandBase.commands
        .get(e.getOption("command", OptionMapping::getAsString)).getValue();
    ...
    if (newStatus == null) {
      cmde.send("commandstatus", new Placeholder("cmd", command.getName()), new Placeholder(
          "status", cmde.getTranslation(guild.isCommandActivated(command) ? "on" : "off")));
    } else {
      if (guild.setCommandActivated(command, newStatus)) {
        final String commandName = command.getName().toUpperCase();
        cmde.success("commandtoggled", "commandisnow", new Placeholder("command", commandName),
            new Placeholder("status", cmde.getTranslation(newStatus ? "on" : "off")));
      } else {
