...
public class CustomCommandsCommand extends TextCommand {
	...
	private static void askForRaw(final String command, final CommandEvent cmde) {
		cmde.getMessage()
				.replyEmbeds(cmde.success().addField("messagetosend", "inputmessage", false)
				...
				.queue(msg -> Bot.getInstance().getEventWaiter().waitForEvent(MessageReceivedEvent.class,
						...
						e -> {
							...
							askForReply(command, new CommandEvent(e, cmde.getGuild(), e.getMessage(), cmde.getMember(),
									cmde.getUser()), customCommand);
						}));
	}

	private static void askForReply(final String command, final CommandEvent cmde, final CustomCommand customCommand) {
		cmde.getMessage()
				.replyEmbeds(cmde.success().addField("shouldreply", "shouldanswer", false)
						.setDescription(cmde.getTranslation("leavetutorial"))
						.setAuthor(cmde.getTranslation("customcommandsetup", new Placeholder("cmd", command)),
								cmde.getJda().getSelfUser().getAvatarUrl())
						.build())
				.queue(msg -> Bot.getInstance().getEventWaiter().waitForEvent(MessageReceivedEvent.class,
						...
						e -> {
							...
							cmde.success("commandadded", "executetutorial",
									new Placeholder("%cmd%", cmde.getGuild().getPrefix() + customCommand.getCommand()));
						}));
	}
	...
	private void askForDelete(final String command, final CommandEvent cmde) {
		cmde.getMessage()
				.replyEmbeds(cmde.success().addField("areyousure", "@blaumeise was soll hier stehen?", false).build())
				.queue(msg -> Bot.getInstance().getEventWaiter().waitForEvent(MessageReceivedEvent.class,
