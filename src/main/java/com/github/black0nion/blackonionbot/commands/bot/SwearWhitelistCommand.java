  ...
  public void execute(final String[] args, final CommandEvent cmde, final MessageReceivedEvent e,
    ...
    if (args.length >= 3
      ...
      for (int i = 2; i < args.length; i++) {
        ...
        final Role rl =
            roles.stream().filter(r -> r.getAsMention().equals(input)).findFirst().orElse(null);
        final TextChannel ch =
            channels.stream().filter(c -> c.getAsMention().equals(input)).findFirst().orElse(null);
      ...
      if (mentionedStuff.size() != 0) {
        ...
        cmde.success("whitelistupdated",
            (add ? cmde.getTranslation("addedtowhitelist",
                new Placeholder("add", mentionedStuff.toString()))
                : cmde.getTranslation("removedfromwhitelist",
                    new Placeholder("removed", mentionedStuff.toString()))));
      }
    } else {
      final List<String> whitelist = guild.getAntiSwearWhitelist();
      cmde.success("antiswearwhitelist",
          (whitelist != null && whitelist.size() != 0 ? whitelist.toString() : "empty"));
    }
  }
}
