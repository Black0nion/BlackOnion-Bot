...
public class BanUsageCommand extends SlashCommand {
  ...
  private static final SubcommandData[] data = {
      new SubcommandData("user", "(un)ban a user").addOption(OptionType.USER, "targetuser",
          "user to (un)ban", true),
      new SubcommandData("guild", "(un)ban a guild").addOption(OptionType.STRING, "targetguild",
          "guild to (un)ban", true),};

  public BanUsageCommand() {
    super(builder(Commands.slash("usage", "(un)ban a user or a guild from using commands.")
        .addSubcommandGroups(
            new SubcommandGroupData("ban", "Ban a user or a guild from using commands.")
                .addSubcommands(data),
            new SubcommandGroupData("unban", "Unban a user or a guild from using commands.")
                .addSubcommands(data))).setHidden()
                    .setRequiredCustomPermissions(CustomPermission.BAN_USAGE));
  }

  public static final MongoCollection<Document> collection =
      MongoDB.DATABASE.getCollection("usagebans");
