...
public class UserInfoCommand extends TextCommand {
  ...
  public void execute(final String[] args, final CommandEvent cmde, final MessageReceivedEvent e,
      final Message message, final BlackMember eventMember, final BlackUser author,
    ...
    if (message.getMentionedMembers().size() > 0) {
      ...
      e.getJDA().retrieveUserById(args[1]).queue(
          idUser -> guild.retrieveMember(idUser).queue(
              member -> cmde.reply(getUserInfo(cmde, Objects.requireNonNull(BlackUser.from(idUser)),
                  BlackMember.from(member, guild))),
              error -> cmde
                  .reply(getUserInfo(cmde, Objects.requireNonNull(BlackUser.from(idUser)), null))),
          new ErrorHandler()
              .handle(ErrorResponse.UNKNOWN_USER, error -> cmde.error("notfound", "usernotfound"))
  ...
  private static EmbedBuilder getUserInfo(final CommandEvent cmde, final BlackUser statsUser,
      final BlackMember statsMember) {
    final String[] flags =
        statsUser.getFlags().stream().map(UserFlag::getName).toArray(String[]::new);
    ...
    builder.addField("language",
        statsUser.getLanguage() != null
            ? statsUser.getLanguage().getName() + " (" + statsUser.getLanguage().getLanguageCode()
                + ")"
            : "nopreference",
        true);
