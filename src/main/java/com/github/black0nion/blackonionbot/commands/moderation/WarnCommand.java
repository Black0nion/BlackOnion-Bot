...
public class WarnCommand extends TextCommand {
  ...
  public WarnCommand() {
    this.setCommand("warn").setRequiredPermissions(Permission.KICK_MEMBERS)
        .setSyntax("<@User> [reason]").setRequiredArgumentCount(1);
  }
    ...
    if (mentionedMembers.size() != 0) {
      ...
      if (args[1].replace("!", "").equalsIgnoreCase(memberToWarn.getAsMention())) {
        if (member.canInteract(memberToWarn)) {
          ...
          cmde.success("userwarned", "usergotwarned",
              new Placeholder("user", memberToWarn.getAsMention()),
