	...
	public void execute(final String[] args, final CommandEvent cmde, final MessageReceivedEvent e,
		...
		try {
			...
			if (args.length >= 2) {
				...
				if (category != null) {
					final EmbedBuilder builder = cmde.success()
							.setTitle(cmde.getTranslation("help") + " | " + category.name());
			...
			if (!(ex instanceof IllegalArgumentException)) {
				...
				ex.printStackTrace();
				message.replyEmbeds(cmde.error().addField("What just happend?", "how, just how", false).build())
						.queue();
			}
		}
	}
				...
				event -> {
					...
					if (Objects.equals(button.getId(), "overview")) {
						...
						for (int i = 0; i <= cats.length; i++) {
							...
							Category category = null;
							if (i == 0) {
								commandsInCategory = new StringBuilder(
										", " + cmde.getTranslationOrEmpty("helpmodules"));
							} else {
								category = cats[i - 1];
								for (final TextCommand c : CommandBase.commandsInCategory.get(category)) {
									if (c.isVisible(user)) {
							...
							builder.addField(
									Utils.firstLetterUppercase((category != null
											? category.name()
							...
							if (entry.getValue().isVisible(user) && (entry.getValue().getCategory() == category))
								if (entry.getValue().getProgress() == Progress.DONE) {
									builder.addField(CommandEvent.getCommandHelp(guild, entry.getValue()),
											cmde.getTranslationOrEmpty("help" + entry.getValue().getCommand()[0]),
											false);
								}
						...
						for (final Progress pr : Progress.values()) {
							...
							for (final Map.Entry<String[], TextCommand> entry : CommandBase.commandsArray.entrySet()) {
								...
								if (command.isVisible(user) && (command.getCategory() == category)
									...
									final String commandHelp = cmde
											.getTranslation("help" + entry.getValue().getCommand()[0].toLowerCase());
									if (commandHelp == null) {
