  ...
  public void execute(final String[] args, final CommandEvent cmde, final MessageReceivedEvent e,
    ...
    try {
      ...
      if (args.length >= 2) {
        ...
        for (final Map.Entry<String[], TextCommand> entry : CommandBase.commandsArray.entrySet()) {
          ...
          if (cmd.isVisible(author) && Arrays.asList(entry.getKey()).contains(args[1])) {
            cmde.success("help", CommandEvent.getCommandHelp(guild, cmd),
                cmde.getTranslationOrEmpty("help" + cmd.getCommand()[0].toLowerCase()));
            return;
          }
        }
        ...
        if (category != null) {
          final EmbedBuilder builder =
              cmde.success().setTitle(cmde.getTranslation("help") + " | " + category.name());
          for (final TextCommand c : CommandBase.commandsInCategory.get(category)) {
            builder.addField(CommandEvent.getCommandHelp(guild, c),
                cmde.getTranslationOrEmpty("help" + c.getCommand()[0]), false);
          }
          cmde.reply(builder);
        } else {
          cmde.error("commandnotfound", "thecommandnotfound",
              new Placeholder("command", "`" + args[1] + "`"));
        }
      } else {
        ...
        for (int i = 0; i <= cats.length; i++) {
          ...
          if (i == 0) {
            ...
            if (CommandBase.commandsInCategory.containsKey(category)) {
              ...
              System.out.println("wtf:  " + category);
          }
          if (commandsInCategory.length() <= 2) {
            continue;
          }
          final String categoryName = Utils.firstLetterUppercase(
              (category != null ? category.name() : cmde.getTranslation("modules")).toLowerCase());
          if (category != null) {
            builder.addField(categoryName, commandsInCategory.substring(1), false);
            buttons.add(Button.primary(category.name(), categoryName));
          } else {
            builder.addField(cmde.getTranslation("modules"), commandsInCategory.substring(1),
                false);
        ...
        channel.sendMessageEmbeds(builder.build())
            .setActionRows(Lists.partition(buttons, 5).stream().map(ActionRow::of).toList())
      ...
      if (!(ex instanceof IllegalArgumentException)) {
        ...
        message
            .replyEmbeds(
                cmde.error().addField("What just happend?", "how, just how", false).build())
            .queue();
      }
    }
  }

  private void waitForHelpCatSelection(final Message msg, final BlackMember author,
      final CommandEvent cmde) {
    Bot.getInstance().getEventWaiter().waitForEvent(ButtonInteractionEvent.class,
        event -> msg.getTextChannel().getIdLong() == event.getChannel().getIdLong()
            && msg.getIdLong() == event.getMessageIdLong() && !event.getUser().isBot()
            && event.getUser().getIdLong() == author.getIdLong(),
        event -> {
          ...
          if (Objects.equals(button.getId(), "overview")) {
            builder.setTitle(LanguageSystem.getTranslation("help", user, guild) + " | "
                + LanguageSystem.getTranslation("modules", user, guild));
            ...
            for (int i = 0; i <= cats.length; i++) {
              ...
              if (i == 0) {
                commandsInCategory =
                    new StringBuilder(", " + cmde.getTranslationOrEmpty("helpmodules"));
              } else {
                category = cats[i - 1];
                for (final TextCommand c : CommandBase.commandsInCategory.get(category)) {
                  if (c.isVisible(user)) {
              ...
              builder.addField(
                  Utils.firstLetterUppercase((category != null ? category.name()
                      : LanguageSystem.getTranslation("modules", user, guild)).toLowerCase()),
                  commandsInCategory.substring(1), false);
            }
            ...
            for (final Map.Entry<String[], TextCommand> entry : CommandBase.commandsArray
                .entrySet())
              if (entry.getValue().isVisible(user) && (entry.getValue().getCategory() == category))
                if (entry.getValue().getProgress() == Progress.DONE) {
                  builder.addField(CommandEvent.getCommandHelp(guild, entry.getValue()),
                      cmde.getTranslationOrEmpty("help" + entry.getValue().getCommand()[0]), false);
                }
            ...
            for (final Progress pr : Progress.values()) {
              ...
              for (final Map.Entry<String[], TextCommand> entry : CommandBase.commandsArray
                ...
                if (command.isVisible(user) && (command.getCategory() == category)
                    && command.getProgress() == pr) {
                  final String commandHelp =
                      cmde.getTranslation("help" + entry.getValue().getCommand()[0].toLowerCase());
                  if (commandHelp == null) {
                    System.out.println("Help for " + entry.getKey()[0] + " not set!");
                  }
                  builder.addField(
                      pr.name().toUpperCase() + ": "
                          + CommandEvent.getCommandHelp(guild, entry.getValue()),
                      cmde.getTranslationOrEmpty("help" + entry.getValue().getCommand()[0]), false);
                }
              }
            }
        ...
        () -> msg
            .editMessage(cmde.getTranslation("helpmenuexpired",
                new Placeholder("cmd", cmde.getGuild().getPrefix() + this.getCommand()[0])))
            .setEmbeds().setActionRows().queue());
  }
}
