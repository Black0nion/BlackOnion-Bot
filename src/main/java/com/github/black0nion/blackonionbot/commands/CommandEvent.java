...
public class CommandEvent {
  ...
  public CommandEvent(final TextCommand cmd, final MessageReceivedEvent e) {
    this(cmd, e, BlackGuild.from(e.getGuild()), e.getMessage(), BlackMember.from(e.getMember()),
        BlackUser.from(e.getAuthor()));
  }

  public CommandEvent(final MessageReceivedEvent e, final BlackGuild guild, final Message message,
      final BlackMember member, final BlackUser user) {
    this(null, e, guild, message, member, user);
  }

  public CommandEvent(final TextCommand cmd, final MessageReceivedEvent e, final BlackGuild guild,
      final Message message, final BlackMember member, final BlackUser user) {
    this.command = cmd;
    this.event = e;
    this.jda = e.getJDA();
    this.guild = guild;
  ...
  public void success(String title, String name, String value,
      final Consumer<? super Message> success, final Placeholder... placeholders) {
  ...
  public void error(final String name, final String value,
      final Consumer<? super Message> success) {
  ...
  public void reply(final EmbedBuilder builder, final Consumer<? super Message> success,
      final Consumer<? super Throwable> error) {
  ...
  public static String getCommandHelp(final BlackGuild guild, final TextCommand command) {
    ...
    return guild.getPrefix() + command.getCommand()[0]
        + (syntax != null && !syntax.equalsIgnoreCase("") ? " " + syntax : "");
  }

  public EmbedBuilder getWrongArgument() {
    return this.error().addField("wrongargument", getPleaseUse(this.guild, this.user, this.command),
        false);
  }
