...
public class StatsCommand extends SlashCommand {
  ...
  public void execute(SlashCommandEvent cmde, SlashCommandInteractionEvent e, BlackMember member,
    ...
    try {
      ...
      cmde.reply(cmde.success().setTitle("Bot Stats")
          .addField("prefix", "`" + guild.getPrefix() + "`", true)
          .addField("runmode", Config.run_mode.name().toUpperCase(), true)
          .addField("os", OS_NAME, true).addField("cpuname", CPU_NAME, true)
          ...
          .addField("commands", CommandBase.commandsArray.size() + SlashCommandBase.commands.size(),
              true)
