...
public class StatsCommand extends SlashCommand {
	...
	public void execute(SlashCommandEvent cmde, SlashCommandInteractionEvent e, BlackMember member, BlackUser author,
		...
		try {
			...
			cmde.reply(cmde.success().setTitle("Bot Stats").addField("prefix", "`" + guild.getPrefix() + "`", true)
					.addField("runmode", Config.run_mode.name().toUpperCase(), true).addField("os", OS_NAME, true)
					.addField("cpuname", CPU_NAME, true).addField("cpucores", OSBEAN.getAvailableProcessors(), true)
					.addField("cpuspeed", CPU_MHZ, true).addField("lines", metadata.lines_of_code(), true)
					...
					.setThumbnail(e.getJDA().getSelfUser().getAvatarUrl() + "?size=512").setTimestamp(Instant.now()));
