...
public class LanguageCommand extends SlashCommand {
	...
	private static final OptionData languageOptions = new OptionData(OptionType.STRING, "language",
			"The code of the language to set", true)
					.addChoices(LanguageSystem.getLanguages().values().stream()
							.map(e -> new Command.Choice(e.getName(), e.getLanguageCode())).toList());

	private static final SubcommandData[] subcommands = {new SubcommandData("get", "Get the current language"),
	...
	public void execute(SlashCommandEvent cmde, SlashCommandInteractionEvent e, BlackMember member, BlackUser author,
		...
		if (subcommand.equalsIgnoreCase("set"))
			Checks.notNull(
					lang = LanguageSystem.getLanguageFromName(e.getOption("language", OptionMapping::getAsString)),
					"Language");
