...
public class SlashCommandEvent {
  ...
  public SlashCommandEvent(final SlashCommandInteractionEvent e, final BlackGuild guild,
      final BlackMember member, final BlackUser user) {
    this(null, e, guild, member, user);
  }

  public SlashCommandEvent(final SlashCommand cmd, final SlashCommandInteractionEvent e,
      final BlackGuild guild, final BlackMember member, final BlackUser user) {
    this.command = cmd;
    this.event = e;
    this.jda = e.getJDA();
    this.guild = guild;
  ...
  public void success(final String title, final String name, final String value,
      final Consumer<InteractionHook> msg) {
  ...
  public void exception(@Nullable Throwable t) {
    if (t != null && !(t instanceof DummyException))
      LoggerFactory.getLogger(this.getClass()).error("Exception in command", t);
    this.send("errorwithmessage",
        new Placeholder("msg",
            t != null
                ? (t instanceof DummyException ? "" : t.getClass().getSimpleName() + ": ")
                    + t.getMessage()
                : "null"));
  }
  ...
  public void reply(final EmbedBuilder builder, boolean ephemeral,
      final Consumer<InteractionHook> result) {
    this.reply(builder.build(), ephemeral, result);
  }

  public void reply(final MessageEmbed embed, boolean ephemeral,
      final Consumer<InteractionHook> result) {
    try {
  ...
  public void send(final String message, final Consumer<InteractionHook> result,
    ...
    this.event.reply(getTranslation(message, placeholders)).setEphemeral(this.command.isEphemeral())
        .queue(result);
  }

  public void send(final Message message, final Consumer<InteractionHook> result) {
    this.event.reply(message).setEphemeral(this.command.isEphemeral()).queue(result,
        System.err::println);
  }
  ...
  public static String getPleaseUse(final BlackGuild guild, final BlackUser author,
  ...
  public static String getCommandHelp(final SlashCommand command) {
    return command.getData().getOptions().stream()
        .map(data -> data.getName() + " - " + data.getDescription() + " : " + data.getType().name())
  ...
  public void handlePerms(Permission... permissions) {
    if (!this.member.hasPermission(permissions)) {
      this.send("missingpermissions",
          new Placeholder("perms", Utils.getPermissionString(permissions)));
