...
public class GuildTypeCommand extends SlashCommand {
		...
		super(builder(Commands.slash("guildtype", "Set the type of a guild")
				...
				.addOptions(new OptionData(OptionType.STRING, "guildtype", "The type of the guild to set")
						.addChoices(Arrays.stream(GuildType.values())
								.map(type -> new Command.Choice(type.name(), type.name())).toList()))).setAdminGuild());
	}
	...
	public void execute(SlashCommandEvent cmde, SlashCommandInteractionEvent e, BlackMember member, BlackUser author,
		...
		long guildID = Utils.getOrReplaceMessage(
				() -> Long.parseLong(Objects.requireNonNull(e.getOption("guildid", OptionMapping::getAsString))),
				"Invalid guild ID");
