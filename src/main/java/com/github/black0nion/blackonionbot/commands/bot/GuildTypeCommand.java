...
public class GuildTypeCommand extends SlashCommand {
  ...
  public GuildTypeCommand() {
    super(builder(Commands.slash("guildtype", "Set the type of a guild")
        .addOption(OptionType.STRING, "guildid", "The ID of the guild to get or set the type of",
            true)
        .addOptions(new OptionData(OptionType.STRING, "guildtype", "The type of the guild to set")
            .addChoices(Arrays.stream(GuildType.values())
                .map(type -> new Command.Choice(type.name(), type.name())).toList())))
                    .setAdminGuild());
  }
  ...
  public void execute(SlashCommandEvent cmde, SlashCommandInteractionEvent e, BlackMember member,
    ...
    long guildID = Utils.getOrReplaceMessage(
        () -> Long
            .parseLong(Objects.requireNonNull(e.getOption("guildid", OptionMapping::getAsString))),
        "Invalid guild ID");
    ...
    if (mentionedBlackGuild != null) {
      if (newGuildType != null) {
        ...
        if (parsedGuildType != null) {
          ...
          cmde.send("guildtypesetto",
              new Placeholder("guild",
                  mentionedBlackGuild.getEscapedName() + " (" + mentionedBlackGuild.getId() + ")"),
              new Placeholder("guildtype", parsedGuildType.name()));
        } else
          throw new IllegalArgumentException("Invalid guild type");
      } else {
        cmde.send("guildtypeis",
            new Placeholder("guild",
                mentionedBlackGuild.getEscapedName() + " (" + mentionedBlackGuild.getId() + ")"),
            new Placeholder("guildtype", mentionedBlackGuild.getGuildType().name()));
      }
    } else {
