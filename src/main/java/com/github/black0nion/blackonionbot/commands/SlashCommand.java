...
public abstract class SlashCommand extends GenericCommand {
  ...
  public abstract void execute(final SlashCommandEvent cmde, final SlashCommandInteractionEvent e,
      final BlackMember member, final BlackUser author, final BlackGuild guild,
  ...
  protected void updateAutoComplete(String option, Collection<String> values) {
    ...
    autoCompletes.put(option,
        values instanceof StartsWithArrayList value ? value : new StartsWithArrayList(values));
  }
  ...
  public void handleAutoComplete(CommandAutoCompleteInteractionEvent event) {
    ...
    List<String> options =
        autoComplete.getElementsStartingWith(event.getFocusedOption().getValue(), true);
    event.replyChoices(options.stream().map(m -> new Command.Choice(m, m)).limit(25).toList())
        .queue();
  }
  ...
  public String toString() {
    return "SlashCommand [data=" + this.data + ", category=" + this.category + ", progress="
        + this.progress + ", requiredPermissions=" + Arrays.toString(this.requiredPermissions)
        + ", requiredBotPermissions=" + Arrays.toString(this.requiredBotPermissions)
        + ", requiredCustomPermissions=" + Arrays.toString(this.requiredCustomPermissions)
        + ", isToggleable=" + this.isToggleable + ", shouldAutoRegister=" + this.shouldAutoRegister
