...
public class JokeCommand extends SlashCommand {
  ...
  public void execute(SlashCommandEvent cmde, SlashCommandInteractionEvent e, BlackMember member,
    ...
    Bot.getInstance().getHttpClient().sendAsync(
        HttpRequest.newBuilder(URI
            .create("https://v2.jokeapi.dev/joke/Any?blacklistFlags=nsfw,racist,sexist&type=twopart"
                + (langParam != null ? langParam : "")))
            .build(),
        HttpResponse.BodyHandlers.ofString()).thenApply(HttpResponse::body).thenAccept(response -> {
          final JSONObject object = new JSONObject(response);
          cmde.success("Joke", "https://jokeapi.dev", object.getString("setup"),
