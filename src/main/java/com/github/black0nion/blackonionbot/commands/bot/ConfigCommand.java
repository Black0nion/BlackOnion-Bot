...
public class ConfigCommand extends TextCommand {
	...
	public ConfigCommand() {
		...
		setters.clear();
		both.clear();
		this.setCommand("config").setSyntax("<list | get | set | clear> [config name (only not required for list)]")
		...
		try {
			...
			for (final Entry<String, Pair<Method, ConfigGetter>> entry : getters.entrySet()) {
				if (setters.containsKey(entry.getKey())) {
					...
					both.put(entry.getKey(),
							new Quadruple<>(getter.getKey(), getter.getValue(), setter.getKey(), setter.getValue()));
	...
	public void execute(final String[] args, final CommandEvent cmde, final MessageReceivedEvent e,
		...
		if (mode.equalsIgnoreCase("list")) {
			...
			@Nonnull
			final Pair<Method, ConfigGetter> finalGetter = onlyGetter != null
					? onlyGetter
			...
			@Nonnull
			final Pair<Method, ConfigSetter> finalSetter = onlySetter != null
					? onlySetter
	...
	private static ConfigSetResponse saveValue(final Object objectToInvokeMethodIn, final Method method,
			final Consumer<Object[]> response, final Object... args) {
		try {
			final Parameter[] parameters = method.getParameters();
