...
public class ConfigCommand extends TextCommand {
  ...
  public ConfigCommand() {
    ...
    this.setCommand("config")
        .setSyntax("<list | get | set | clear> [config name (only not required for list)]")
    ...
    try {
      ...
      for (final Entry<String, Pair<Method, ConfigGetter>> entry : getters.entrySet()) {
        if (setters.containsKey(entry.getKey())) {
          ...
          both.put(entry.getKey(), new Quadruple<>(getter.getKey(), getter.getValue(),
              setter.getKey(), setter.getValue()));
  ...
  public static final HashMap<String, Pair<Method, ConfigGetter>> getters = new HashMap<>();
  public static final HashMap<String, Pair<Method, ConfigSetter>> setters = new HashMap<>();
  public static final HashMap<String, Quadruple<Method, ConfigGetter, Method, ConfigSetter>> both =
      new HashMap<>();
    ...
    if (mode.equalsIgnoreCase("list")) {
      ...
      if (getters.size() != 0) {
        help += "Read-only:\n" + getters.entrySet().stream()
            .map(en -> "- " + en.getKey() + " | " + en.getValue().getValue().description())
      ...
      if (both.size() != 0) {
        help += "Get and Setable:\n" + both.entrySet().stream()
            .map(en -> "- " + en.getKey() + " | " + en.getValue().getSecond().description())
            .collect(Collectors.joining("\n"));
      }
      help += "```";
      ...
      @Nullable
      final Quadruple<Method, ConfigGetter, Method, ConfigSetter> bothGetterAndSetter =
          both.get(key);
      ...
      @Nonnull
      final Pair<Method, ConfigGetter> finalGetter = onlyGetter != null ? onlyGetter
          : new Pair<>(bothGetterAndSetter.getFirst(), bothGetterAndSetter.getSecond());
      try {
        ...
        cmde.success("Value for: %for%", "Current Value: %value%",
            new Placeholder("for", finalGetter.getValue().key()),
            new Placeholder("value", invoke == null ? "empty" : result));
      ...
      @Nonnull
      final Pair<Method, ConfigSetter> finalSetter = onlySetter != null ? onlySetter
          : new Pair<>(bothGetterAndSetter.getThird(), bothGetterAndSetter.getFourth());
      ...
      if (mode.equalsIgnoreCase("set")) {
        final ConfigSetResponse saveValue = saveValue(guild, method,
            response -> cmde.success("valueset", "valuesetto %newvalue%",
                new Placeholder("newvalue", arrayToString(response))),
  ...
  private static ConfigSetResponse saveValue(final Object objectToInvokeMethodIn,
      final Method method, final Consumer<Object[]> response, final Object... args) {
    try {
      final Parameter[] parameters = method.getParameters();
      ...
      for (int i = 0; i < args.length; i++) {
        ...
        if (parameterType == String.class) {
          ...
          if (args[i] == null)
            return ConfigSetResponse.NOSUCHCHANNEL;
        } else if (parameterType == long.class || parameterType == Long.class) {
          if (!Utils.isLong(args[1]))
            return ConfigSetResponse.NOTANUMBER;
          ...
          if (clazz == String.class) {
            ...
            LoggerFactory.getLogger(ConfigCommand.class)
                .error("No array casting way provided for " + clazz);
          }
          break;
        } else {
          parsed[i] = Dashboard.getValue(parameterType, args[i]);
        }
        if ((parsed[i] == null || (Utils.isLong(parsed[i]) && (Long) parsed[i] == -1)))
          throw new IllegalArgumentException(
              "args[" + i + "] is null, should be of type " + parameterType.getName() + "!");
      }
      if (hasArray) {
