...
public class ConfigCommand extends Command {
	...
	public void execute(final String[] args, final CommandEvent cmde, final MessageReceivedEvent e, final Message message, final BlackMember member, final BlackUser author, final BlackGuild guild, final TextChannel channel) {
		...
		if (mode.equalsIgnoreCase("list")) {
			...
			@Nullable final Pair<Method, ConfigGetter> onlyGetter = getters.get(key);
			@Nullable final Quadruple<Method, ConfigGetter, Method, ConfigSetter> bothGetterAndSetter = both.get(key);
			...
			final String key = args[2];
			@Nullable final Pair<Method, ConfigSetter> onlySetter = setters.get(key);
			@Nullable final Quadruple<Method, ConfigGetter, Method, ConfigSetter> bothGetterAndSetter = both.get(key);
