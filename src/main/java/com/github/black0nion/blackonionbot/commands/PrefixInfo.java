...
public class PrefixInfo {
  ...
  public static void handle(final CommandEvent cmde) {
    ...
    if (author.isBot() || message.getMentionedUsers().size() == 0)
      return;
      ...
      if (args.length >= 3) {
        if (args[1].equalsIgnoreCase("prefix")) {
          if (member.hasPermission(Permission.MANAGE_SERVER)) {
            ...
            cmde.error("missingpermissions", "requiredpermissions",
                new Placeholder("permission", Utils.getPermissionString(Permission.ADMINISTRATOR)));
          }
          return;
        }
      }
      cmde.success("myprefixis", "changeprefix", new Placeholder("prefix", guild.getPrefix()),
          new Placeholder("command", "<@!" + BotInformation.SELF_USER_ID + "> prefix <prefix>"));
    }
  }
}
