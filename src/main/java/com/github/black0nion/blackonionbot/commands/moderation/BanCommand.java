...
public class BanCommand extends SlashCommand {
	...
	public BanCommand() {
		super(builder(Commands.slash("ban", "Used to ban a user from the server.")
				...
				.addOptions(new OptionData(OptionType.INTEGER, DEL_DAYS, "The amount of days to delete messages from the user.", true).addChoices(deletionDays)))
			.setRequiredPermissions(Permission.BAN_MEMBERS)
			.setRequiredBotPermissions(Permission.BAN_MEMBERS));
	}

	//region Deletion Days
	private static final List<Command.Choice> deletionDays = List.of(
