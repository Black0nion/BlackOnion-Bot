...
public class HangmanCommand extends TextCommand {
  ...
  public void execute(final String[] args, final CommandEvent cmde, final MessageReceivedEvent e,
    ...
    final String solution =
        wordsInThisLang.get(ThreadLocalRandom.current().nextInt(wordsInThisLang.size()));
  ...
  private static void rerun(final Message msg, final CommandEvent cmde, final String solution,
      final List<Character> alreadyGuessed) {
    final String failedAttempts = getFailedAttempts(solution, alreadyGuessed);
    final int failedAttemptsCount =
        failedAttempts.equalsIgnoreCase("") ? 0 : failedAttempts.split(", ").length;

    if (failedAttemptsCount >= 7) {
      msg.editMessageEmbeds(
          cmde.error().setTitle("hangman").addField("urded", "notbigsurprise", false).build())
          .queue();
      ingamePlayers.remove(cmde.getUser().getIdLong());
      return;
    }

    final EmbedBuilder builder = cmde.success().setTitle("hangman")
        .setDescription("```\n" + getSpacesString(solution, alreadyGuessed) + "\nFailed Attempts: "
            + failedAttempts + "\n" + getDrawing(failedAttemptsCount) + "```");
    if (won(solution, alreadyGuessed)) {
      msg.editMessageEmbeds(
          cmde.success().setTitle("hangman").addField("uwon", "bigsurprise", false).build())
          .queue();
      ingamePlayers.remove(cmde.getUser().getIdLong());
      return;
    }
    ...
    msg.editMessageEmbeds(builder.build()).queue(
        message -> Bot.getInstance().getEventWaiter().waitForEvent(MessageReceivedEvent.class,
            event -> event.getChannelType() == ChannelType.TEXT
                && event.getGuild().getIdLong() == cmde.getGuild().getIdLong()
                && event.getAuthor().getIdLong() == cmde.getUser().getIdLong()
                && !event.getMessage().getContentRaw().toLowerCase().startsWith("!")
                && !alreadyGuessed
                    .contains(event.getMessage().getContentRaw().toLowerCase().charAt(0)),
            event -> {
              if (event.getMessage().getContentRaw().equalsIgnoreCase(solution)) {
                msg.editMessageEmbeds(cmde.success().setTitle("hangman")
                    .addField("uwon", "bigsurprise", false).build()).queue();
                ingamePlayers.remove(cmde.getUser().getIdLong());
                return;
              }
              ...
              msg.editMessageEmbeds(
                  cmde.success().addField("timeout", "tooktoolong", false).build()).queue();
              ingamePlayers.remove(cmde.getUser().getIdLong());
            }));
  }
