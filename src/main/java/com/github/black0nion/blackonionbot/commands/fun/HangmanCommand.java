...
public class HangmanCommand extends TextCommand {
	...
	private static void rerun(final Message msg, final CommandEvent cmde, final String solution,
		...
		if (failedAttemptsCount >= 7) {
			msg.editMessageEmbeds(cmde.error().setTitle("hangman").addField("urded", "notbigsurprise", false).build())
					.queue();
		...
		if (won(solution, alreadyGuessed)) {
			msg.editMessageEmbeds(cmde.success().setTitle("hangman").addField("uwon", "bigsurprise", false).build())
					.queue();
			ingamePlayers.remove(cmde.getUser().getIdLong());
			return;
		}
		...
		msg.editMessageEmbeds(builder.build())
				.queue(message -> Bot.getInstance().getEventWaiter().waitForEvent(MessageReceivedEvent.class,
						event -> event.getChannelType() == ChannelType.TEXT
								...
								&& !alreadyGuessed.contains(event.getMessage().getContentRaw().toLowerCase().charAt(0)),
						event -> {
							...
							msg.editMessageEmbeds(cmde.success().addField("timeout", "tooktoolong", false).build())
									.queue();
							ingamePlayers.remove(cmde.getUser().getIdLong());
						}));
	}
