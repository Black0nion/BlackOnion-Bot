...
public class QrDecodeCommand extends TextCommand {
	...
	private void send(final CommandEvent cmde, final Result result, final String imageUrl) {
		if (result == null || result.getBarcodeFormat() == null) {
			...
			cmde.reply(cmde.success().setTitle("qrcode", "https://zxing.github.io/zxing").setThumbnail(imageUrl)
					.addField("qrresult", result.getText(), false));
		}
	}

	private void readQR(final CommandEvent cmde, final String url, final BlackUser author, final BlackGuild guild,
			final Message msg) {
		try {
			final BinaryBitmap binaryBitmap = new BinaryBitmap(
					new HybridBinarizer(new BufferedImageLuminanceSource(ImageIO.read(new URL(url).openStream()))));
			...
			if (e instanceof NotFoundException) {
				msg.replyEmbeds(
						EmbedUtils.getErrorEmbed(author, guild).addField("qrdecodefail", "qrnotfound", false).build())
						.queue();
