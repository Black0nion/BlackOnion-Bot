...
public class QrDecodeCommand extends TextCommand {
  ...
  public QrDecodeCommand() {
    this.setCommand("qrdecode", "qrd").setSyntax("<attach file / url to a public image>");
  }
  ...
  private void send(final CommandEvent cmde, final Result result, final String imageUrl) {
    if (result == null || result.getBarcodeFormat() == null) {
      ...
      cmde.reply(cmde.success().setTitle("qrcode", "https://zxing.github.io/zxing")
          .setThumbnail(imageUrl).addField("qrresult", result.getText(), false));
    }
  }

  private void readQR(final CommandEvent cmde, final String url, final BlackUser author,
      final BlackGuild guild, final Message msg) {
    try {
      final BinaryBitmap binaryBitmap = new BinaryBitmap(new HybridBinarizer(
          new BufferedImageLuminanceSource(ImageIO.read(new URL(url).openStream()))));

      final Result result = multiFormatReader.decode(binaryBitmap);
      this.send(cmde, result, url);
    } catch (final Exception e) {
      if (e instanceof NotFoundException) {
        msg.replyEmbeds(EmbedUtils.getErrorEmbed(author, guild)
            .addField("qrdecodefail", "qrnotfound", false).build()).queue();
      } else if (e.getMessage().startsWith("Server returned HTTP response code: 403 for URL:")) {
        msg.replyEmbeds(EmbedUtils.getErrorEmbed(author, guild)
            .addField("privateqr", "uploadasattachment", false).build()).queue();
      } else {
        e.printStackTrace();
        msg.replyEmbeds(EmbedUtils.getErrorEmbed(author, guild)
  ...
  private void readQR(final CommandEvent cmde, final Attachment path, final BlackGuild guild,
      final Message msg) {
    try {
      final InputStream is = path.retrieveInputStream().join();
      final BinaryBitmap binaryBitmap =
          new BinaryBitmap(new HybridBinarizer(new BufferedImageLuminanceSource(ImageIO.read(is))));
      is.close();
      ...
      if (e instanceof NotFoundException) {
        msg.replyEmbeds(EmbedUtils.getErrorEmbed(BlackUser.from(msg.getMember()), guild)
            .addField("qrdecodefail", "qrnotfound", false).build()).queue();
      } else if (e.getMessage().startsWith("Server returned HTTP response code: 403 for URL:")) {
        msg.replyEmbeds(EmbedUtils.getErrorEmbed(BlackUser.from(msg.getMember()), guild)
            .addField("privateqr", "uploadasattachment", false).build()).queue();
