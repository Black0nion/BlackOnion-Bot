...
public class SkipCommand extends TextCommand {
  ...
  public void execute(final String[] args, final CommandEvent cmde, final MessageReceivedEvent e,
    ...
    if (state != null && state.getChannel() != null) {
      ...
      final AudioTrackInfo previousTrack =
          musicManager.scheduler.player.getPlayingTrack().getInfo();
      if (musicManager.scheduler.queue.peek() != null) {
        ...
        cmde.success("songskipped", "songgotskipped",
            new Placeholder("oldsong",
                Utils.escapeMarkdown(previousTrack.author + " - " + previousTrack.title)),
            new Placeholder("newsong",
                Utils.escapeMarkdown(newTrack.author + " - " + newTrack.title)));
      } else {
