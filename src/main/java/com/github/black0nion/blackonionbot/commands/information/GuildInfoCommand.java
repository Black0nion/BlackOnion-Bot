...
public class GuildInfoCommand extends TextCommand {
  ...
  public void execute(final String[] args, final CommandEvent cmde, final MessageReceivedEvent e,
    ...
    cmde.reply(cmde.success().setTitle("guildinfo").setThumbnail(guild.getIconUrl())
        .addField("name", guild.getEscapedName(), true)
        .addField("language", guild.getLanguage() != null
            ? (guild.getLanguage().getName() + " (" + guild.getLanguage().getLanguageCode() + ")")
            : "none", true)
