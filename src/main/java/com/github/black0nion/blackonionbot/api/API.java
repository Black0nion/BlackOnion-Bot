...
public class API {
  ...
  public static void init() {
    ...
    final Set<Class<? extends IWebSocketEndpoint>> websocketsClasses =
        reflections.getSubTypesOf(IWebSocketEndpoint.class);
    ...
    // region Map Requests
    final Set<Class<? extends IHttpRoute>> requestClasses =
        reflections.getSubTypesOf(IHttpRoute.class);
    ...
    ExceptionHandler<Exception> exceptionHandler = (e, ctx) -> {
      ...
      @Nullable
      final HttpResponseException http =
          e instanceof HttpResponseException eAsHttp ? eAsHttp : null;
      ctx.status(http != null ? http.getStatus() : 500)
      ...
      if (ctx.status() == 429) {
        logger.warn("IP {} exceeded rate limit for {} which is {}", ctx.ip(), ctx.path(),
            ...
            http != null && http.getMessage() != null
                ? http.getMessage().replaceFirst("Rate limit exceeded - Server allows ", "")
                    .replace(".", "")
                : "unknown");
      }
    };
    ...
    for (final IHttpRoute req : httpRoutes) {
      ...
      final Handler handler = ctx -> {
        ...
        if (req.requiresLogin()) {
          ...
          if (sessionId == null)
            throw new UnauthorizedResponse("No SessionID provided");
          if (!sessionId.matches(BlackSession.SESSIONID_REGEX))
            throw new UnauthorizedResponse("Invalid SessionID");
          try {
        ...
        Object result =
            req.handle(ctx, body, headers, session, session != null ? session.getUser() : null);
