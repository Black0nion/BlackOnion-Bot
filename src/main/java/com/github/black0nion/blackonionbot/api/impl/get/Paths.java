...
public class Paths implements IGetRoute {
	...
	public Object handle(Context ctx, JSONObject body, Map<String, String> headers, @Nullable BlackSession session,
		...
		return new JSONObject(collected).put("WS",
				new JSONArray(instance.wsHandlerMetaInfoList.stream().map(WsHandlerMetaInfo::getPath).toList()));
	}
	...
	/**
	 * A listener that listens for all the paths that are registered to the Javalin
	 * instance. Taken from {@link io.javalin.core.util.RouteOverviewConfig
	 * RouteOverviewConfig} and {@link io.javalin.core.util.RouteOverviewUtil
	 * RouteOverviewUtil}
	 * 
