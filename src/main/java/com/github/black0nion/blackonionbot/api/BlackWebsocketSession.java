...
public class BlackWebsocketSession extends BlackSession
    implements org.eclipse.jetty.websocket.api.Session {

  public BlackWebsocketSession(final Session sessionRaw)
      throws InputMismatchException, ExecutionException {
  ...
  public void close(final int statusCode, final String reason) {
    logger.info("IP {} will get closed with code {} and reason {}",
        this.session.getRemote().getInetSocketAddress().getAddress().getHostAddress(), statusCode,
        reason);
  ...
  public void disconnect() throws IOException {
    logger.info("IP {} disconnected",
        this.session.getRemote().getInetSocketAddress().getAddress().getHostAddress());
