...
public class BlackSession {
  ...
  public static final MongoCollection<Document> collection =
      MongoDB.DATABASE.getCollection("dashboard-sessions");
  ...
  /**
   * The websocket will connect and then provide a session id to log in to. If the websocket
   * connection isn't logged in, use {@link OAuthUtils#loginWithDiscord(String)}
   * <p>
   * Auth flow:
   * <ul>
   ...
   * </ul>
   */
  public void loginToSession(final String sessionId)
      throws ExecutionException, InputMismatchException, NullPointerException {
    ...
    this.user =
        OAuthUtils.getUserWithToken(doc.getString("access_token"), doc.getString("refresh_token"));
  }
  ...
  public static String generateSessionId() {
    ...
    if (collection.find(Filters.eq("sessionid", generatedId)).first() != null)
