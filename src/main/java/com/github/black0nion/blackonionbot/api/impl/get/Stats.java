...
public class Stats implements IGetRoute {
	...
	public Object handle(Context ctx, JSONObject body, Map<String, String> headers, @Nullable BlackSession session,
		...
		return new JSONObject().put("commands", CommandBase.commandsArray.size() + SlashCommandBase.commands.size())
				.put("code_stats",
						new JSONObject().put("line_count", Config.metadata.lines_of_code()).put("file_count",
				...
				.put("cpu",
						new JSONObject().put("cpu_name", CPU_NAME).put("cpu_cores", OSBEAN.getAvailableProcessors())
								.put("cpu_speed", CPU_MHZ))
				.put("guild_count", StatisticsManager.getGuildCount())
				.put("user_count", StatisticsManager.getUserCount()).put("prefix", Config.prefix).put("os", OS_NAME)
				.put("uptime", System.currentTimeMillis() - StatisticsManager.STARTUP_TIME)
				.put("version", metadata.version()).put("run_mode", Config.run_mode.name().toUpperCase())
