...
/**
 * Logs out a user from a session (deletes their session id from the database and revoke token)
 *
 * @author _SIM_
 */
public class Logout implements IPostRoute {
  ...
  public Object handle(Context ctx, JSONObject body, Map<String, String> headers,
    ...
    Bson filter = Filters.eq("sessionid",
        assertMatches(ctx.header("sessionid"), BlackSession.SESSIONID_REGEX));
