...
public class BlackUser extends UserImpl {
	...
	private BlackUser(final User user) {
		...
		try {
			...
			this.permissions = Utils.gOD(
					CustomPermission.parse(Utils.gOD(config.getList("permissions", String.class), new ArrayList<>())),
					new ArrayList<>());
	...
	private static final MongoCollection<Document> configs = MongoManager.getCollection("usersettings",
			MongoDB.DATABASE);
