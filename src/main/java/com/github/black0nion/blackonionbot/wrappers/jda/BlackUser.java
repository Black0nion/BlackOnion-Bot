...
public class BlackUser extends UserImpl {
  ...
  private static final LoadingCache<User, BlackUser> users =
  ...
  private BlackUser(final User user) {
    ...
    try {
      ...
      this.permissions = Utils.gOD(
          CustomPermission
              .parse(Utils.gOD(config.getList("permissions", String.class), new ArrayList<>())),
          new ArrayList<>());
  ...
  public void setPermissions(final List<CustomPermission> permissions) {
    ...
    this.save("permissions",
        permissions.stream().map(CustomPermission::name).collect(Collectors.toList()));
  }
  ...
  private static final MongoCollection<Document> configs =
      MongoManager.getCollection("usersettings", MongoDB.DATABASE);
  ...
  public String toString() {
    return "BlackUser{" + "fullName=" + this.getFullName() + "language=" + language
