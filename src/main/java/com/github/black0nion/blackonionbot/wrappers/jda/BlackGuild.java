...
public class BlackGuild extends GuildImpl {
	...
	private BlackGuild(@NotNull final Guild guild) {
		...
		try {
			...
			this.antiSpoilerType = Utils.gOD(AntiSpoilerType.parse(config.getString("antispoiler")),
					AntiSpoilerType.OFF);
			...
			Utils.gOD(
					Utils.gOD(config.getList("customcommands", Document.class), new ArrayList<Document>()).stream()
							.map(cmd -> new CustomCommand(this, cmd)).collect(Collectors.toList()),
					new ArrayList<CustomCommand>()).forEach(cmd -> this.customCommands.put(cmd.getCommand(), cmd));
	...
	@ConfigGetter(key = "leavechannel", requiredPermissions = {
			Permission.MANAGE_SERVER}, description = "Set the Channel the Message on leave should be sent to")
	...
	@ConfigSetter(argumentsDescription = {
			"The Channel where the message should get sent to"}, key = "leavechannel", requiredPermissions = {
					Permission.MANAGE_SERVER}, nullable = true)
	...
	public void setDisabledCommands(final String[] disabledCommands) {
		this.setDisabledCommands(Arrays.stream(disabledCommands)
				.map(cmd -> Optional.ofNullable((GenericCommand) CommandBase.commands.get(cmd)).orElse(
						Optional.ofNullable(SlashCommandBase.commands.get(cmd)).map(Pair::getValue).orElse(null)))
	...
	public boolean addCustomCommand(final CustomCommand cmd) {
		...
		this.save("customcommands",
				this.customCommands.values().stream().map(CustomCommand::toDocument).collect(Collectors.toList()));
		return true;
	}
	...
	public String toString() {
		return "BlackGuild{" + "selfBlackMember=" + selfBlackMember + ", language=" + language + ", guildType="
				+ guildType + ", antiSpoilerType=" + antiSpoilerType + ", antiSwearType=" + antiSwearType
				+ ", antiSwearWhitelist=" + antiSwearWhitelist + ", prefix='" + prefix + '\'' + ", joinMessage='"
				+ joinMessage + '\'' + ", joinChannel=" + joinChannel + ", leaveMessage='" + leaveMessage + '\''
				+ ", leaveChannel=" + leaveChannel + ", disabledCommands=" + disabledCommands + ", suggestionsChannel="
				+ suggestionsChannel + ", autoRoles=" + autoRoles + ", loop=" + loop + ", customCommands="
				+ customCommands + '}';
	}
}
