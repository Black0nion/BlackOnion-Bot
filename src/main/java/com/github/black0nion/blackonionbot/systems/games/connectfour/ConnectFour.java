...
public class ConnectFour {
  ...
  public ConnectFour(final MessageChannel channel, final User playerX, final User playerY) {
    ...
    channel
        .sendMessageEmbeds(EmbedUtils.getSuccessEmbed()
            .setTitle("Connect 4 | Aktueller Spieler: " + Utils.escapeMarkdown(
                (this.currentUser == FieldType.X ? playerX.getName() : playerY.getName())))
            .addField("Current State:", this.getFieldString(), false).build())
        .queue(success -> this.messageID = success.getIdLong());
  ...
  public FieldType getWinner() {
    if (this.won(FieldType.X))
      return FieldType.X;
    else if (this.won(FieldType.O))
      return FieldType.O;
  ...
  public boolean won(final FieldType player) {
    ...
    for (int j = 0; j < ConnectFourGameManager.WIDTH - 3; j++) {
      for (int i = 0; i < ConnectFourGameManager.HEIGHT; i++)
        if (this.field[i][j] == player && this.field[i][j + 1] == player
            && this.field[i][j + 2] == player && this.field[i][j + 3] == player)
          return true;
    }
    // verticalCheck
    for (int i = 0; i < ConnectFourGameManager.HEIGHT - 3; i++) {
      for (int j = 0; j < ConnectFourGameManager.WIDTH; j++)
        if (this.field[i][j] == player && this.field[i + 1][j] == player
            && this.field[i + 2][j] == player && this.field[i + 3][j] == player)
          return true;
    }
    // ascendingDiagonalCheck
    for (int i = 3; i < ConnectFourGameManager.HEIGHT; i++) {
      for (int j = 0; j < ConnectFourGameManager.WIDTH - 3; j++)
        if (this.field[i][j] == player && this.field[i - 1][j + 1] == player
            && this.field[i - 2][j + 2] == player && this.field[i - 3][j + 3] == player)
          return true;
    }
  ...
  public boolean isValidInput(final String input) {
    ...
    try {
      ...
      if (Utils.alphabet.contains(charInput[0])
          && Utils.alphabet.indexOf(charInput[0]) < ConnectFourGameManager.WIDTH && numbAtOne >= 0
          && numbAtOne < ConnectFourGameManager.HEIGHT)
        return true;
    ...
    for (int y = 0; y < ConnectFourGameManager.HEIGHT; y++) {
      for (int x = 0; x < ConnectFourGameManager.WIDTH; x++) {
        output.append("\\| ").append(
            this.field[y][x] == FieldType.EMPTY ? "    " : " " + this.field[y][x].name() + " ");
      }
