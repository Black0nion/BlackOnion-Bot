...
public class GiveawaySystem {
	...
	Bot.executor.submit(() -> {
	    ...
	    for (final Document doc : collection.find()) if (doc.keySet().containsAll(giveawayKeys)) {
	...
	try {
	    ...
	    if (giveaways.contains(giveaway)) return giveaway;
	...
	try {
		...
		if (users.size() == 0 || users.stream().filter(user -> (user.getIdLong() != selfUser.getIdLong())).count() == 0) {
	...
	try {
	    ...
	    try {
		collection.deleteOne(new BasicDBObject().append("messageId", giveaway.getMessageId()).append("guildId", giveaway.getGuildId()).append("channelId", giveaway.getChannelId()));
	    } catch (final Exception ex) {
		ex.printStackTrace();
	    }
	} catch (final Exception e) {
