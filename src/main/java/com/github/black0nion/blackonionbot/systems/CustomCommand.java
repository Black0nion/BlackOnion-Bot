...
public class CustomCommand {
  ...
  public CustomCommand(final BlackGuild guild, final Document doc) {
    ...
    if (doc.containsKey("embed")) {
      ...
      System.err.println("CustomCommand " + this.getCommand() + " in Guild "
          + this.getGuild().getName() + " has no handler set!");
    }
  ...
  public void handle(final MessageReceivedEvent event) {
    if (this.embed != null) {
      ...
      this.embed.setFooter(author.getName() + "#" + author.getDiscriminator(),
      ...
      throw new NullPointerException("Both embed and Answer is null!");
  }
  ...
  public Document toDocument() {
    ...
    if (this.embed != null) {
      ...
      if (fields.size() != 0) {
        ...
        fields.forEach(field -> fieldsDoc
            .add(new Document().append("name", field.getName()).append("value", field.getValue())));
  ...
  public boolean equals(final Object obj) {
    ...
    return cmd.getCommand().equalsIgnoreCase(this.getCommand())
        && cmd.getGuild().getIdLong() == this.getGuild().getIdLong();
  }
}
