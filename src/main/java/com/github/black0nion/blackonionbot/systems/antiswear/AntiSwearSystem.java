...
public class AntiSwearSystem {
	...
	public static boolean check(final BlackGuild guild, final BlackMember author, final Message message,
		...
		if (Utils.handleRights(guild, user, channel, Permission.MESSAGE_MANAGE))
			return false;
		...
		try {
			...
			if (whitelist != null && (whitelist.contains(channel.getAsMention())
					|| author.getRoles().stream().anyMatch(role -> whitelist.contains(role.getAsMention()))))
				return false;
			// Message messageRaw = event.getMessage();
			HttpResponse<String> response = Bot.getInstance().getHttpClient().send(HttpRequest.newBuilder(URI.create(
					"https://westeurope.api.cognitive.microsoft.com/contentmoderator/moderate/v1.0/ProcessText/Screen?autocorrect=false&classify=True"))
					.POST(HttpRequest.BodyPublishers.ofString(messageContent)).header("Content-Type", "text/plain")
					.header("Ocp-Apim-Subscription-Key", Config.content_moderator_token).build(),
			...
			if (responseJson.has("Terms")) {
				...
				if (!(responseJson.get("Terms") instanceof JSONArray))
					return false;

				StatisticsManager.PROFANITY_FILTERED
				...
				try {
					...
					if (type == REPLACE) {
						...
						if (Utils.handleRights(guild, user, channel, Permission.MANAGE_WEBHOOKS))
							return true;
						...
						channel.sendMessageEmbeds(EmbedUtils.getErrorEmbed(user, guild)
				...
				throw new RuntimeException(
						"Some error happened while contacting the Microsoft api. Response: \n" + responseBody);
		} catch (final Exception e) {
			e.printStackTrace();
		}
		return false;
