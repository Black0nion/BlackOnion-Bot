...
public class PluginSystem {
  ...
  private static void loadPlugin(final File jarFile) {
    ...
    final ScanResult scanResult =
        new ClassGraph().enableAnnotationInfo().overrideClasspath(jarFile.getAbsolutePath()).scan();

    final String superclass = Plugin.class.getName();

    final Optional<ClassInfo> optionalClassInfo = scanResult.getAllClasses().stream()
        .filter(classInfo -> classInfo.extendsSuperclass(superclass)).findFirst();
    ...
    try {
      final Plugin plugin = (Plugin) Class
          .forName(classInfo.getName(), true,
      ...
      PluginInformation information =
          new PluginInformation(plugin, new BlackThread(threadGroup, () -> {
            try {
              new BlackThread(plugin::onEnable).setThreadName("[PLUGIN MAIN] " + pluginName)
                  .startThread().join();
            } catch (Exception e) {
      ...
      logger.info("The Plugin \"{}\" stored in file \"{}\" got loaded successfully!", pluginName,
          jarName);
    ...
    plugins.forEach((plugin, info) -> {
      ...
      new BlackThread(threadGroup, () -> {
        try {
          ...
          BlackThread disableThread = new BlackThread(threadGroup, plugin::onDisable)
              .setThreadName("[ONDISABLE] " + plugin).startThread();
          Thread.sleep(4000);
          Thread pluginThread = info.getMainThread();
          if (pluginThread.isAlive()) {
            info.setState(PluginState.DISABLING_NOW);
            BlackThread disableNowThread = new BlackThread(threadGroup, plugin::onDisableNow)
                .setThreadName("[ONDISABLENOW] " + plugin).startThread();
            Thread.sleep(1000);
            if (disableNowThread.isAlive() || disableThread.isAlive() || pluginThread.isAlive()) {
              if (disableNowThread.isAlive())
                disableNowThread.interrupt();
