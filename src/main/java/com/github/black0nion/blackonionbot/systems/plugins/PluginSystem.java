...
public class PluginSystem {
	...
	private static void loadPlugin(final File jarFile) {
		...
		final ScanResult scanResult = new ClassGraph().enableAnnotationInfo()
				.overrideClasspath(jarFile.getAbsolutePath()).scan();
		...
		try {
			final Plugin plugin = (Plugin) Class
					.forName(classInfo.getName(), true, new URLClassLoader(new URL[]{jarFile.toURI().toURL()}))
			...
			PluginInformation information = new PluginInformation(plugin, new BlackThread(threadGroup, () -> {
				try {
					new BlackThread(plugin::onEnable).setThreadName("[PLUGIN MAIN] " + pluginName).startThread().join();
				} catch (Exception e) {
