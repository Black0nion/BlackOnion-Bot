...
public class JoinLeaveSystem extends ListenerAdapter {
	...
	public void onGuildMemberJoin(final @NotNull GuildMemberJoinEvent event) {
		try {
			...
			channel.sendMessage(guild.getJoinMessage().replace("%user%", author.getAsMention()).replace("%guild%",
					guild.getEscapedName())).addFile(file, "welcome.png").queue();
		} catch (final Exception e) {
			e.printStackTrace();
		}
	}
	...
	public void onGuildMemberRemove(final @NotNull GuildMemberRemoveEvent event) {
		try {
			...
			channel.sendMessage(guild.getLeaveMessage().replace("%user%", author.getAsMention()).replace("%guild%",
					guild.getEscapedName())).addFile(file, "goodbye.png").queue();
		} catch (final Exception e) {
			e.printStackTrace();
		}
	}
	...
	public void onGuildJoin(final @NotNull GuildJoinEvent event) {
		Bot.getInstance().getExecutor().submit(() -> {
			...
			guild.retrieveOwner().queue(user -> {
				...
				logger.info("I got added to the guild {} (G: {}) with owner {} (U: {})", guild.getName(), guild.getId(),
							...
							.sendMessageEmbeds(EmbedUtils.getSuccessEmbed().addField("addedtoguild",
									LanguageSystem.getDefaultLanguage().getTranslation("guildstatsjoin",
											new Placeholder("name",
													guild.getEscapedName() + "(G:" + guild.getId() + ")"),
				...
				if (Config.run_mode == RunMode.BETA && !guild.getGuildType().higherThanOrEqual(GuildType.BETA)) {
					guild.leave().queue();
					author.openPrivateChannel().queue(channel -> channel.sendMessageEmbeds(
							EmbedUtils.getErrorEmbed(author, guild).addField("notbeta", "betatutorial", false).build())
							.queue());
				...
				author.openPrivateChannel()
						.queue(channel -> channel
								.sendMessageEmbeds(EmbedUtils.getSuccessEmbed(author, guild)
	...
	public void onGuildLeave(final GuildLeaveEvent event) {
		Bot.getInstance().getExecutor().submit(() -> {
			...
			try {
				final Guild guildById = event.getJDA().getGuildById(BotInformation.supportServer);
				guildById.getTextChannelById(BotInformation.botLogsChannel)
						.sendMessageEmbeds(EmbedUtils.getErrorEmbed().addField("removedfromguild",
	...
	public static File generateImage(@NotNull final Color textColor, final @NotNull BlackUser user,
		...
		final String message = drawType == DrawType.JOIN
				? LanguageSystem.getTranslation("welcome", user, guild)
