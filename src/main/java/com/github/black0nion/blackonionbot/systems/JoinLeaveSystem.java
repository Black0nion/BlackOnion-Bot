...
public class JoinLeaveSystem extends ListenerAdapter {
  ...
  public JoinLeaveSystem() {
    try {
      defaultBackGround =
          ImageIO.read(Objects.requireNonNull(this.getClass().getResource("/background.png")));
      return;
  ...
  public void onGuildMemberJoin(final @NotNull GuildMemberJoinEvent event) {
    try {
      ...
      channel.sendMessage(guild.getJoinMessage().replace("%user%", author.getAsMention())
          .replace("%guild%", guild.getEscapedName())).addFile(file, "welcome.png").queue();
    } catch (final Exception e) {
      e.printStackTrace();
    }
  }
  ...
  public void onGuildMemberRemove(final @NotNull GuildMemberRemoveEvent event) {
    try {
      ...
      channel.sendMessage(guild.getLeaveMessage().replace("%user%", author.getAsMention())
          .replace("%guild%", guild.getEscapedName())).addFile(file, "goodbye.png").queue();
    } catch (final Exception e) {
      e.printStackTrace();
    }
  }
  ...
  public void onGuildJoin(final @NotNull GuildJoinEvent event) {
    Bot.getInstance().getExecutor().submit(() -> {
      ...
      guild.retrieveOwner().queue(user -> {
        ...
        logger.info("I got added to the guild {} (G: {}) with owner {} (U: {})", guild.getName(),
            guild.getId(), author.getName(), author.getId());

        try {
          final Guild guildById = event.getJDA().getGuildById(BotInformation.supportServer);
          guildById.getTextChannelById(BotInformation.botLogsChannel)
              .sendMessageEmbeds(EmbedUtils.getSuccessEmbed().addField("addedtoguild",
                  LanguageSystem.getDefaultLanguage().getTranslation("guildstatsjoin",
                      new Placeholder("name", guild.getEscapedName() + "(G:" + guild.getId() + ")"),
                      new Placeholder("usercount", guild.getMemberCount()),
                      new Placeholder("owner",
                          author.getEscapedName() + "(U:" + author.getId() + ")")),
                  false).build())
              .queue();
        } catch (final Exception e) {
          e.printStackTrace();
        ...
        if (Config.run_mode == RunMode.BETA
            && !guild.getGuildType().higherThanOrEqual(GuildType.BETA)) {
          guild.leave().queue();
          author.openPrivateChannel()
              .queue(channel -> channel.sendMessageEmbeds(EmbedUtils.getErrorEmbed(author, guild)
                  .addField("notbeta", "betatutorial", false).build()).queue());
          logger.error("{} (G: {}) added me but is not a beta guild!", guild.getName(),
              guild.getId());
          return;
        }
        ...
        author.openPrivateChannel()
            .queue(channel -> channel
                .sendMessageEmbeds(
                    EmbedUtils.getSuccessEmbed(author, guild).setTitle("thankyouforadding")
                        .addField(
                            LanguageSystem.getTranslation("commandtohelp", author, guild)
                                .replace("%command%", prefix + "help"),
                            LanguageSystem.getTranslation("changelanguage", author, guild)
                                .replace("%usercmd%", prefix + "lang")
                                .replace("%guildcmd%", prefix + "guildlang"),
                            false)
                        .build())
                .queue());
      });
  ...
  public void onGuildLeave(final GuildLeaveEvent event) {
    Bot.getInstance().getExecutor().submit(() -> {
      ...
      try {
        final Guild guildById = event.getJDA().getGuildById(BotInformation.supportServer);
        guildById.getTextChannelById(BotInformation.botLogsChannel)
            .sendMessageEmbeds(EmbedUtils.getErrorEmbed()
                .addField("removedfromguild",
                    LanguageSystem.getDefaultLanguage().getTranslation("guildstatsleave",
                        new Placeholder("name",
                            Utils.escapeMarkdown(guild.getName()) + "(G:" + guild.getId() + ")"),
                        new Placeholder("usercount", guild.getMemberCount())),
                    false)
                .build())
            .queue();
  ...
  @NotNull
  public static File generateImage(@NotNull final Color textColor, final @NotNull BlackUser user,
      final @NotNull BlackGuild guild, final DrawType drawType) throws Exception {
    ...
    try {
      ...
      innerBox.setComposite(
          AlphaComposite.getInstance(AlphaComposite.SRC_OVER, (float) separatorTransparency));
    ...
    if (user.getEffectiveAvatarUrl() != null) {
      ...
      try {
        avatar.setRenderingHint(RenderingHints.KEY_INTERPOLATION,
            RenderingHints.VALUE_INTERPOLATION_BILINEAR);
        avatar.drawImage(makeRoundedCorner(ImageIO.read(connection.getInputStream())), 10, 10, 180,
            180, null);
      } finally {
        avatar.dispose();
    ...
    newGraphics.setRenderingHint(RenderingHints.KEY_TEXT_ANTIALIASING,
        RenderingHints.VALUE_TEXT_ANTIALIAS_ON);
    newGraphics.setFont(new Font("Arial", Font.PLAIN, 60));
    newGraphics.setColor(Color.BLACK);
    final String message =
        drawType == DrawType.JOIN ? LanguageSystem.getTranslation("welcome", user, guild)
            : LanguageSystem.getTranslation("goodbye", user, guild);
