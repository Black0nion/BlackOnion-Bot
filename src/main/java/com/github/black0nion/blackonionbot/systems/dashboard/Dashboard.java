...
public class Dashboard {
  ...
  public static void init() {
    ...
    final Set<Class<? extends BlackWrapper>> annotated =
        reflections.getSubTypesOf(BlackWrapper.class);
  ...
  public static void tryUpdateValue(final JSONObject message, final DiscordUser user,
      final Consumer<ResponseCode> callback) {
  ...
  public static boolean saveValue(final Object objectToInvokeMethodIn, final Method method,
      final Object... args) {
    try {
      ...
      for (int i = 0; i < args.length; i++) {
        ...
        if ((parsed[i] == null || (Utils.isLong(parsed[i]) && (Long) parsed[i] == -1))
            && !nullableSetters.contains(method))
