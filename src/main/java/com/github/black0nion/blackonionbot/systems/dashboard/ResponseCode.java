...
public enum ResponseCode {
  ...
  SUCCESS(true), FAIL("epic fail bru bru"), NO_ACTION(
      "No action specified."), WRONG_ARGUMENTS, UNAUTHORIZED, JSON_ERROR(
          "Invalid JSON Syntax."), NO_GUILD("No guild specified."), INVALID_TYPE(
              "This type doesn't exist."), WRONG_SETTING(
                  "This setting doesn't exist."), NO_PERMISSIONS(
                      "You are not allowed to do that."), PARSE_ERROR(
                          "A argument has the wrong type."), LOGGED_IN(true, "You got logged in.");
  ...
  ResponseCode(final boolean success, final String message) {
    this.json =
        new JSONObject().put("code", (success ? "S_" : "E_") + this.name()).put("message", message);
  }
