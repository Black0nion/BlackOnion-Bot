...
public class TrackScheduler extends AudioEventAdapter {
  ...
  public void onTrackStart(final AudioPlayer player, final AudioTrack track) {
    ...
    if (!(this.queue.size() == 1 && this.guild.loopActivated())) {
      this.guild.getTextChannelById(MusicSystem.channels.get(this.guild.getIdLong()))
          .sendMessageEmbeds(EmbedUtils.getSuccessEmbed(null, this.guild)
              .setTitle(LanguageSystem.getTranslation("nowplaying", null, this.guild) + info.title,
                  info.uri)
