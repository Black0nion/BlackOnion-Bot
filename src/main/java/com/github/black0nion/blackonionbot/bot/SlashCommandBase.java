...
public class SlashCommandBase extends ListenerAdapter {
  ...
  public static final HashMap<Class<? extends SlashCommand>, SlashCommand> commandInstances =
      new HashMap<>();
  ...
  public static void addCommands() {
    ...
    for (final Class<?> command : annotated) {
      try {
        ...
        newInstance
            .setCategory(parsedCategory != null ? parsedCategory : newInstance.getCategory());
        ...
        if (newInstance.getRequiredCustomPermissions() == null
          ...
          final JSONObject commandJSON =
              new JSONObject().put("name", data.getName()).put("description", data.getDescription())
          ...
          for (SubcommandGroupData subcommandGroupData : data.getSubcommandGroups()) {
            final JSONObject subcommandGroupJson =
                new JSONObject().put("name", subcommandGroupData.getName()).put("description",
                    subcommandGroupData.getDescription());
    ...
    Optional.ofNullable(getCommand(ToggleCommand.class))
        .ifPresent(ToggleCommand::updateAutoComplete);
  ...
  public static void updateCommandsDev(JDA jda) {
    if (Config.dev_guild != -1) {
      Objects.requireNonNull(jda.getGuildById(Config.dev_guild)).updateCommands()
          .addCommands(
              commands.values().stream().map(Pair::getValue).map(SlashCommand::getData).toList())
          .queue();
    }
  }
  ...
  public void onSlashCommandInteraction(final SlashCommandInteractionEvent event) {
    ...
    final String log = EmojiParser.parseToAliases(guild.getName() + "(G:" + guild.getId() + ") > "
        ...
        + author.getDiscriminator() + "(U:" + author.getId() + "): (M:" + event.getId() + ")"
        + event.getCommandPath() + " " + event.getOptions().stream().map(OptionMapping::toString)
    ...
    if (Config.run_mode == RunMode.DEV) {
      if (locked)
        ...
        logger.info(log);
      FileUtils.appendToFile("files/logs/messagelog/" + guild.getId() + "/"
    ...
    if (locked) {
      // haha funni
      event.reply(
          "https://tenor.com/view/you-got-banned-banned-banned-message-mickey-mouse-club-gif-17668002")
          .queue();
      return;
    }
    ...
    if (Utils.handleRights(guild, author, channel, Permission.VIEW_CHANNEL,
    ...
    if (commands.containsKey(event.getName())) {
      ...
      StatisticsManager.COMMANDS_EXECUTED.labels("slash", event.getCommandPath(), guild.getId(),
  ...
  public static void addCommand(final SlashCommand c) {
    ...
    if (!commands.containsKey(commandName)) {
      ...
      final List<SlashCommand> commandsInCat =
          Optional.ofNullable(commandsInCategory.get(category)).orElseGet(ArrayList::new);
      commandsInCat.add(c);
