...
public class SlashCommandBase extends ListenerAdapter {
	...
	public static void addCommands() {
		...
		for (final Class<?> command : annotated) {
			try {
				...
				if (newInstance.getRequiredCustomPermissions() == null
						|| newInstance.getRequiredCustomPermissions().length == 0) {
					final JSONObject commandJSON = new JSONObject().put("name", data.getName())
					...
					for (SubcommandGroupData subcommandGroupData : data.getSubcommandGroups()) {
						...
						for (SubcommandData subcommandData : subcommandGroupData.getSubcommands()) {
							JSONObject subcommandObjJson = new JSONObject().put("name", subcommandData.getName())
					...
					for (SubcommandData subcommandData : data.getSubcommands()) {
						subcommandJson.put(subcommandData.getName(),
								new JSONObject().put("name", subcommandData.getName())
										.put("description", subcommandData.getDescription())
				...
				// should be covered by the builder
					...
					throw new RuntimeException("Command " + command.getName() + " has no data!");
			} catch (Exception e) {
				e.printStackTrace();
	...
	public static void updateCommandsDev(JDA jda) {
		if (Config.dev_guild != -1) {
			Objects.requireNonNull(jda.getGuildById(Config.dev_guild)).updateCommands()
					.addCommands(commands.values().stream().map(Pair::getValue).map(SlashCommand::getData).toList())
					.queue();
		}
	}
	...
	public void onSlashCommandInteraction(final SlashCommandInteractionEvent event) {
		...
		final boolean locked = BanUsageCommand.collection
				.find(Filters.or(Filters.eq("guildid", guild.getIdLong()), Filters.eq("userid", author.getIdLong())))
				.first() != null;
		final String log = EmojiParser.parseToAliases(
				guild.getName() + "(G:" + guild.getId() + ") > " + channel.getName() + "(C:" + channel.getId() + ") | "
						+ author.getName() + "#" + author.getDiscriminator() + "(U:" + author.getId() + "): (M:"
						+ event.getId() + ")" + event.getCommandPath() + " " + event.getOptions().stream()
								.map(OptionMapping::toString).collect(Collectors.joining(" ")).replace("\n", "\\n"));
		...
		if (Config.run_mode == RunMode.DEV) {
			if (locked)
				...
				logger.info(log);
			FileUtils.appendToFile("files/logs/messagelog/"
					+ guild.getId() + "/" + EmojiParser.parseToAliases(channel.getName())
							.replaceAll(":([^:\\s]*(?:::[^:\\s]*)*):", "($1)").replace(":", "_")
					+ "_" + channel.getId() + ".log", log);
		}

		if (locked) {
			// haha funni
			event.reply("https://tenor.com/view/you-got-banned-banned-banned-message-mickey-mouse-club-gif-17668002")
					.queue();
			return;
		}
		...
		if (Utils.handleRights(guild, author, channel, Permission.VIEW_CHANNEL, Permission.MESSAGE_SEND))
			return;
		if (commands.containsKey(event.getName())) {
			...
			StatisticsManager.COMMANDS_EXECUTED.labels("slash", event.getCommandPath(), guild.getId(), guild.getName(),
					channel.getId(), channel.getName()).inc();
			StatisticsManager.TOTAL_COMMANDS_EXECUTED.inc();

			final Permission[] requiredBotPermissions = cmd.getRequiredBotPermissions() != null
					? cmd.getRequiredBotPermissions()
					: new Permission[]{};
			final Permission[] requiredPermissions = cmd.getRequiredPermissions() != null
	...
	public static void addCommand(final SlashCommand c) {
		...
		if (!commands.containsKey(commandName)) {
			...
			final List<SlashCommand> commandsInCat = Optional.ofNullable(commandsInCategory.get(category))
