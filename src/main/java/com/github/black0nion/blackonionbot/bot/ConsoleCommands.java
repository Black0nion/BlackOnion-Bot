    ...
    while (true) {
      try {
        ...
        if (input.equalsIgnoreCase("reload") || input.equalsIgnoreCase("rl")) {
          ...
          boolean loadUser =
              args.length > 1 && Utils.isBoolean(args[1]) && Boolean.parseBoolean(args[1]);
          logger.info("Guilds: " + Bot.getInstance().getJda().getGuilds().stream().map(g -> {
            User user = null;
            if (loadUser)
              user = g.retrieveOwner().submit().join().getUser();
            return g.getName() + "(" + g.getId() + ")"
                + (user != null ? " | " + user.getAsTag() + "(" + user.getId() + ")" : "");
          }));
          ...
          logger.info(
              "Command not recognized. Valid Commands: [reload, shutdown, setlogorigin, setloglevel, peek]");
        }
