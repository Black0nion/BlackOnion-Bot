...
public class CommandBase extends ListenerAdapter {
	...
	public void onMessageReceived(final MessageReceivedEvent event) {
		...
		StatisticsManager.MESSAGES_SENT.labels(guild.getId(), guild.getName(), channel.getId(), channel.getName())
				.inc();
		...
		final String attachmentsString = (!attachments.isEmpty()
				? attachments.stream().map(Attachment::getUrl).toList().toString()
				: "");
		...
		boolean locked = BanUsageCommand.collection
		...
		if (!args[0].startsWith(prefix) && !args[0].equalsIgnoreCase(event.getJDA().getSelfUser().getAsMention()))
			return;
		final String str = args[0].replace(prefix, "").toLowerCase();
		if (Utils.handleRights(guild, author, channel, Permission.MESSAGE_MANAGE, Permission.MESSAGE_SEND))
			return;
		if (commands.containsKey(str)) {
			...
			StatisticsManager.COMMANDS_EXECUTED
					.labels("text", cmd.getName(), guild.getId(), guild.getName(), channel.getId(), channel.getName())
					.inc();
			...
			if (!member.hasPermission(requiredPermissions)) {
				...
				message.replyEmbeds(
						EmbedUtils.getErrorEmbed(author, guild).addField(cmde.getTranslation("wrongargumentcount"),
								CommandEvent.getPleaseUse(guild, author, cmd), false).build())
						.queue(msg -> {
	...
	public static void addCommand(final TextCommand c) {
		if (!commandsArray.containsKey(c.getCommand())) {
			...
			final List<TextCommand> commandsInCat = Optional.ofNullable(commandsInCategory.get(category))
