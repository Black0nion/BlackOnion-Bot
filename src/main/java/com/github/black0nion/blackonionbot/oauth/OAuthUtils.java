...
public class OAuthUtils {
  ...
  public static final DiscordOAuth OAUTH_HANDLER =
      new DiscordOAuth(CLIENT_ID, CLIENT_SECRET, REDIRECT_URI, SCOPES);
  ...
  public static String getTokenURL(String code) {
    return TOKEN_URL + "?client_id=" + CLIENT_ID + "&client_secret=" + CLIENT_SECRET
        + "&grant_type=" + GRANT_TYPE + "&code=" + code + "&redirect_uri=" + REDIRECT_URI;
  }

  public static String getTokenRevokeURL(String token) {
    return TOKEN_REVOKE_URL + "?client_id=" + CLIENT_ID + "&client_secret=" + CLIENT_SECRET
        + "&token=" + token;
  }
  ...
  /**
   * call once to generate token from code and save that shit, only on first login with discord, on
   * reconnect on the same PC (session) use {@link BlackSession#loginToSession(String)}!
   *
  ...
  public static String loginWithDiscord(final String code)
    ...
    if (response == null)
      ...
      final Document find =
          BlackSession.collection.find(Filters.and(Filters.eq("access_token", accessToken),
      ...
      BlackSession.collection.insertOne(
          new Document().append("sessionid", newSessionId).append("access_token", accessToken)
