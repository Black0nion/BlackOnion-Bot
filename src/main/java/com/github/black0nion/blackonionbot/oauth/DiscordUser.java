...
public class DiscordUser {
	...
	public JSONObject getUserAsJson() {
		return new JSONObject().put("id", user.getId()).put("username", user.getUsername())
				...
				.put("verified", user.getVerified()).put("email", user.getEmail()).put("flags", user.getFlags())
	 ...
	 * @throws IOException
	 *             if the request fails - this is a UncheckedIOException because
	 *             Suppliers don't allow you to throw Exceptions
	 */
