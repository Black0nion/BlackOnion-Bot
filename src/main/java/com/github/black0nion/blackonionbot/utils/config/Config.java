...
public class Config {
  ...
  public static final @Nonnull String prefix =
      get("prefix", String.class, defaultValue("*"), matchesRegex("\\w{1,10}"));
  public static Activity.ActivityType activity_type = get("activity_type",
      Activity.ActivityType.class, defaultValue(Activity.ActivityType.PLAYING));
  ...
  public static OnlineStatus online_status =
      get("online_status", OnlineStatus.class, defaultValue(OnlineStatus.ONLINE));
  /**
   * Currently unused, will get added by the slash command branch
   */
  @SuppressWarnings("unused")
  public static final String activity_url =
      get("activity_url", String.class, matchesRegex(Activity.STREAMING_URL));
  public static final String discordapp_client_secret = get("discordapp_client_secret",
      String.class, matchesRegex(Pattern.compile("^[a-z\\d=_\\-]{32}$", Pattern.CASE_INSENSITIVE)));
  public static final String discordapp_client_id =
      get("discordapp_client_id", String.class, matchesRegex("\\d{17,19}"));
  public static final String discordapp_redirect_url =
      get("discordapp_redirect_url", String.class, matchesRegex("https?://.+"));
  public static final String mongo_connection_string =
      get("mongo_connection_string", String.class, Flags.NonNull, matchesRegex(
  ...
  public static final String openweatherapikey =
      get("openweathermap_api_key", String.class, matchesRegex("[a-z\\d]{32}"));
  @Nonnull
  ...
  public static final String spotify_client_id =
      get("spotify_client_id", String.class, matchesRegex("[\\w\\d]{32}"));
  @Nullable
  public static final String spotify_client_secret =
      get("spotify_client_secret", String.class, matchesRegex("[\\w\\d]{32}"));
  public static final int api_port =
      get("api_port", Integer.class, defaultValue(187), range(0, 65535));
  ...
  public static final int prometheus_port =
      get("prometheus_port", Integer.class, defaultValue(9090), range(0, 65535));
  ...
  static <T> T get(String name, Class<T> clazz, IFlag... flagsArr) {
    ...
    final String value =
        System.getenv().containsKey(name) ? System.getenv(name) : System.getProperty(name);
      ...
      if (f instanceof Flags.MatchesRegex flag) {
        if (!flag.regex().matcher(value).matches()) {
          throw new IllegalArgumentException(
              "Config value " + name + " does not match regex " + flag.regex());
        }
      } else if (f instanceof Flags.Range flag) {
        if (result instanceof Number) {
          if (((Number) result).doubleValue() < flag.min()
              || ((Number) result).doubleValue() > flag.max()) {
            throw new IllegalArgumentException(
                "Config value " + name + " is out of range " + flag.min() + " to " + flag.max());
          }
        }
      }
    }
  ...
  private static <T extends IFlag> T getFlag(List<IFlag> flags,
      @SuppressWarnings("SameParameterValue") Class<T> clazz) {
    return flags.stream().filter(Objects::nonNull).filter(f -> clazz.isAssignableFrom(f.getClass()))
