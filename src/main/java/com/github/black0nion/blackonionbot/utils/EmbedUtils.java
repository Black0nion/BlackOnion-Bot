...
public class EmbedUtils {
  ...
  public static TranslatedEmbed getErrorEmbed(final BlackUser author, final BlackGuild guild) {
    final TranslatedEmbed builder = new TranslatedEmbed(LanguageSystem.getLanguage(author, guild))
        .setTitle("error").setColor(Color.RED).setTimestamp(Instant.now());
    if (author != null) {
  ...
  public static TranslatedEmbed getSuccessEmbed(final BlackUser author, final BlackGuild guild) {
    final TranslatedEmbed builder = new TranslatedEmbed(LanguageSystem.getLanguage(author, guild))
        .setTitle("success").setColor(BLACK_ONION_COLOR).setTimestamp(Instant.now());
    if (author != null) {
      builder.setFooter(author.getFullName(), author.getEffectiveAvatarUrl());
    }
    return builder;
  ...
  public static TranslatedEmbed getSuccessEmbed() {
    return new TranslatedEmbed().setTitle("success").setColor(BLACK_ONION_COLOR)
        .setTimestamp(Instant.now());
  }

  public static TranslatedEmbed getLoadingEmbed(final BlackUser author, final BlackGuild guild) {
    final TranslatedEmbed builder = new TranslatedEmbed(LanguageSystem.getLanguage(author, guild))
        .setTitle("loading").setColor(Color.getHSBColor(0.16F, 1F, 1F)).setTimestamp(Instant.now());
    if (author != null) {
      builder.setFooter(author.getFullName(), author.getEffectiveAvatarUrl());
    }
    return builder;
  ...
  public static MessageEmbed premiumRequired(final BlackUser author, final BlackGuild guild) {
    final TranslatedEmbed builder = new TranslatedEmbed(LanguageSystem.getLanguage(author, guild))
        .setTitle("error").addField("notpremium", "premiumrequired", false).setColor(premiumColor)
        .setTimestamp(Instant.now());
    if (author != null) {
      builder.setFooter(author.getFullName(), author.getEffectiveAvatarUrl());
    }
