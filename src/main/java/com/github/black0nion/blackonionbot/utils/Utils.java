...
public class Utils {
  ...
  public static float map(final float value, final float minInput, final float maxInput,
      final float minMapped, final float maxMapped) {
  ...
  public static String escapeMarkdown(final String text) {
    return text.replace("\\(\\*|_|`|~|\\)", "$1").replace("(\\*|_|`|~|\\)", "\\$1");
  }
  ...
  public static String getCountryFromCode(final String code) {
    return new JSONObject(String.join("\n",
        new BufferedReader(new InputStreamReader(
            Objects.requireNonNull(Utils.class.getResourceAsStream("/countrycodes.json")))).lines()
                .collect(Collectors.joining()))).getString(code);
  }
  ...
  public static final String[] emojis = new String[] {":zero:", ":one:", ":two:", ":three:",
   ...
   */
  public static boolean handleRights(final BlackGuild guild, final BlackUser author,
      final TextChannel channel, final Permission... permissions) {
    if (channel == null) {
  ...
  public static MessageEmbed noRights(final BlackGuild guild, final BlackUser author,
      final Permission... missingPermissions) {
    return EmbedUtils.getErrorEmbed(author, guild)
        .addField("idonthavepermissions",
            LanguageSystem.getTranslation("requiredpermissions", author, guild) + "\n"
                + getPermissionString(missingPermissions),
            false)
        .build();
  }
  ...
  public static String getPermissionString(final Permission... permissions) {
    ...
    for (int i = 0; i < permissions.length; i++) {
      output.append("- ").append(permissions[i].getName())
          .append(i == permissions.length - 1 ? "" : "\n");
    }
    return output + "```";
  }
  ...
  public static String getPermissionString(final CustomPermission... permissions) {
    StringBuilder output = new StringBuilder("```");
    for (int i = 0; i < permissions.length; i++) {
      output.append("- ").append(permissions[i].name())
          .append(i == permissions.length - 1 ? "" : "\n");
    }
    return output + "```";
  }
  ...
  public static String parseDate(final long diff) {
    ...
    return (diffDays != 0 ? diffDays + " days" : "")
        ...
        + (diffSeconds != 0 ? " " + diffSeconds + " seconds" : "");
  }
  ...
  public static Webhook getWebhook(TextChannel channel, List<Webhook> webhooks) throws IOException {
    return webhooks.stream().filter(Objects::nonNull)
        .filter(webhook1 -> webhook1.getOwner() != null)
        .filter(webhook1 -> webhook1.getName().equals("BlackOnion-Bot ContentModerator")
            && webhook1.getOwner().getIdLong() == BotInformation.SELF_USER_ID)
        .findFirst()
        .orElse(channel.createWebhook("BlackOnion-Bot ContentModerator")
            .setAvatar(Icon.from(Objects.requireNonNull(
  ...
  public static JSONArray optionsToJson(List<OptionData> options) {
    ...
    for (OptionData option : options) {
      jsonArray.put(
          new JSONObject().put("name", option.getName()).put("description", option.getDescription())
              .put("type", option.getType().name()).put("required", option.isRequired()));
    }
    return jsonArray;
  }
}
