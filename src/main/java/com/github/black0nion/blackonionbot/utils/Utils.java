...
public class Utils {
	...
	public static float map(final float value, final float minInput, final float maxInput, final float minMapped,
			final float maxMapped) {
	...
	public static BufferedImage deepCopy(@NotNull final BufferedImage bufferedImage) {
		...
		final WritableRaster raster = bufferedImage
				.copyData(bufferedImage.getRaster().createCompatibleWritableRaster());
	...
	public static final String[] emojis = new String[]{":zero:", ":one:", ":two:", ":three:", ":four:", ":five:",
	 ...
	 */
	public static boolean handleRights(final BlackGuild guild, final BlackUser author, final TextChannel channel,
	...
	public static MessageEmbed noRights(final BlackGuild guild, final BlackUser author,
		...
		return EmbedUtils.getErrorEmbed(author, guild)
				.addField("idonthavepermissions", LanguageSystem.getTranslation("requiredpermissions", author, guild)
						+ "\n" + getPermissionString(missingPermissions), false)
				.build();
	}
	...
	public static Webhook getWebhook(TextChannel channel, List<Webhook> webhooks) throws IOException {
		return webhooks.stream().filter(Objects::nonNull).filter(webhook1 -> webhook1.getOwner() != null)
	...
	public static JSONArray optionsToJson(List<OptionData> options) {
		...
		for (OptionData option : options) {
			jsonArray.put(new JSONObject().put("name", option.getName()).put("description", option.getDescription())
