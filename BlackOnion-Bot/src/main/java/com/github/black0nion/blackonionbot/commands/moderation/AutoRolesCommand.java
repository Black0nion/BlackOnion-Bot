...
public class AutoRolesCommand implements Command {
	...
	public void execute(String[] args, GuildMessageReceivedEvent e, Message message, Member member, User author, Guild guild, MessageChannel channel) {
		...
		if (args[1].equalsIgnoreCase("create")) {
			...
			channel.sendMessage(EmbedUtils.getSuccessEmbed(author, guild).addField("autorolecreated", LanguageSystem.getTranslation("autorolecreatedinfo", author, guild).replace("%role%", role.getAsMention()), false).build()).queue();
			...
			GuildManager.saveList(guild.getId(), "autoroles", tempList);
			channel.sendMessage(EmbedUtils.getSuccessEmbed(author, guild).addField("autoroledeleted", LanguageSystem.getTranslation("autoroledeletedinfo", author, guild).replace("%role%", role.getAsMention()), false).build()).queue();
		} else {
			channel.sendMessage(EmbedUtils.getErrorEmbed(author, guild).addField("wrongargument", LanguageSystem.getTranslation("pleaseuse", author, guild) + " " + getSyntax(), false).build()).queue();
		}
	}
