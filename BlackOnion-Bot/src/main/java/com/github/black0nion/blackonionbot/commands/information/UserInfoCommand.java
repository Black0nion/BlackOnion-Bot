...
public class UserInfoCommand extends Command {
	...
	public void execute(final String[] args, final CommandEvent cmde, final GuildMessageReceivedEvent e, final BlackMessage message, final BlackMember member, final BlackUser author, final BlackGuild guild, final TextChannel channel) {
		...
		if (message.getMentionedBlackMembers().size() > 0) {
			...
			try { Long.parseLong(args[1]); } catch (final Exception ex) { cmde.error("notfound", "usernotfound"); return; }
	...
	private static final EmbedBuilder getUserInfo(final CommandEvent cmde, final BlackUser statsUser, final BlackMember statsMember) {
		final String[] flags = statsUser.getFlags().stream().map(entry -> entry.getName()).toArray(String[]::new);
