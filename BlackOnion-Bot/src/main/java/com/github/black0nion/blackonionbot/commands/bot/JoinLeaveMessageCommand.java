...
public class JoinLeaveMessageCommand implements Command {
	...
	public void execute(String[] args, GuildMessageReceivedEvent e, Message message, Member member, User author, Guild guild, MessageChannel channel) {
		...
		if (args[1].equalsIgnoreCase("join")) {
			...
			channel.sendMessage(EmbedUtils.getSuccessEmbed(author, guild).addField("setjoinmessage", LanguageSystem.getTranslation("joinmessagesetto", author, guild).replace("%msg%", "``" + newMessage + "``"), false).build()).queue();
		} else if (args[1].equalsIgnoreCase("leave")) {
			GuildManager.save(guild, "leavemessage", newMessage);
			channel.sendMessage(EmbedUtils.getSuccessEmbed(author, guild).addField("setleavemessage", LanguageSystem.getTranslation("leavemessagesetto", author, guild).replace("%msg%", "``" + newMessage + "``"), false).build()).queue();
		} else {
