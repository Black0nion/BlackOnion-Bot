...
public class TicTacToeCommand extends Command {
	...
	CommandBase.waiter.waitForEvent(GuildMessageReceivedEvent.class, answerEvent -> answerEvent.getGuild().getIdLong() == channel.getGuild().getIdLong() && game.isPlayer(answerEvent.getAuthor().getId()), answerEvent -> {
		...
		game.nextUser();
		game.getMessage().editMessageEmbeds(EmbedUtils.getSuccessEmbed().setTitle(getTranslation("tictactoe", author, guild) + " | " + getTranslation("currentplayer", author, guild) + " " + Utils.removeMarkdown((game.currentPlayer == FieldType.X ? game.getPlayerX().getName() : game.getPlayerY().getName()))).addField(getTranslation("currentstate", author, guild), game.getFieldString(), false).build()).queue();
	    ...
	    game.getMessage().editMessageEmbeds(EmbedUtils.getErrorEmbed().addField(guild.getLanguage().getTranslationNonNull("timeout"), guild.getLanguage().getTranslationNonNull("tooktoolong"), false).build()).queue();
	    TicTacToeGameManager.deleteGame(game);
	    return;
	});
    }
	...
	if (firstWinner == null) return false;
	    ...
	    game.getMessage().editMessageEmbeds(EmbedUtils.getSuccessEmbed().addField("WE HAVE A WINNER!", "And the winner is....\n" + Utils.removeMarkdown((firstWinner == FieldType.X ? game.getPlayerX().getAsMention() : game.getPlayerY().getAsMention())) + "!", false).build()).queue();
	    TicTacToeGameManager.deleteGame(game);
	    return true;
	} else {
	    game.getMessage().editMessageEmbeds(EmbedUtils.getSuccessEmbed().addField("WE HAVE NO WINNER!", "u both succ, nobody won, lul", false).build()).queue();
	    TicTacToeGameManager.deleteGame(game);
	    return true;
	}
    }
