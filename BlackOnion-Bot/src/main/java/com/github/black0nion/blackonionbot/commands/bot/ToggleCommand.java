...
public class ToggleCommand implements Command {
	...
	public void execute(String[] args, GuildMessageReceivedEvent e, Message message, Member member, User author, Guild guild, MessageChannel channel) {
		...
		if (ToggleAPI.setActivated(guild.getId(), command, activated)) {
			...
			channel.sendMessage(EmbedUtils.getSuccessEmbed(author, guild).addField(LanguageSystem.getTranslation("commandtoggled", author, guild).replace("%command%", commandName), LanguageSystem.getTranslation("commandisnow", author, guild).replace("%command%", commandName).replace("%status%", LanguageSystem.getTranslation(activated ? "on" : "off", author, guild)), false).build()).queue();
			return;
		} else {
