...
public class BigbrainMemeCommand implements Command {
	...
	public void execute(String[] args, GuildMessageReceivedEvent e, Message message, Member member, User author, Guild guild, MessageChannel channel) {
		...
		if (messages.length < 4) {
			channel.sendMessage(EmbedUtils.getErrorEmbed(author, guild).addField("wrongargumentcount", LanguageSystem.getTranslation("pleaseuse", author, guild) + " " + BotInformation.getPrefix(guild) + getCommand()[0] + " " + getSyntax(), false).build()).queue();
			return;
		}
